// EPUB Reader Logic for Basic Mode
...
async function openEpubReader(localPath, title) {
    try {
        ...
        if (window.electronAPI && window.electronAPI.readEpubFile) {
            ...
            try {
                const res = await fetch(`/api/books/read?path=${encodeURIComponent(localPath)}`);
...
function updateFocusModeUI() {
    ...
    if (isReaderFocusMode) {
        ...
        if (btn) {
            ...
            btn.innerHTML = '<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg> Focus Active';
        }
    } else {
        if (ruler) ruler.style.display = 'none';
        if (btn) {
            btn.classList.remove('active');
            btn.innerHTML = '<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg> Enable Focus';
        }
    }
}
...
document.addEventListener('click', (e) => {
    ...
    if (e.target.closest('#readerSettingsBtn')) {
        ...
        if (!panel.classList.contains('hidden')) applyReaderPrefs();
    }
