...
// Server state (use window.selectedServer when config.js already declared it, e.g. in Umbrella)
let currentMediaData = null;
if (typeof window.selectedServer === 'undefined') {
  window.selectedServer = localStorage.getItem('selectedServer') || 'Videasy';
}
...
function initServers() {
    ...
    if (serverDropdown) {
        ...
        Object.keys(serverConfigs).forEach(serverName => {
            ...
            if (serverName === window.selectedServer) {
...
function showServerSelection(mediaData) {
    ...
    if (serverDropdown) {
        serverDropdown.value = window.selectedServer;
    }
...
function setupServerEventListeners() {
    ...
    if (serverDropdown) {
        serverDropdown.addEventListener('change', (e) => {
            window.selectedServer = e.target.value;
            localStorage.setItem('selectedServer', window.selectedServer);
            console.log('[Servers] Selected server:', window.selectedServer);
        });
    }
    ...
    if (serverWatchBtn) {
        serverWatchBtn.addEventListener('click', () => {
            ...
            const currentSelectedServer = serverDropdown ? serverDropdown.value : window.selectedServer;
...
window.umbrellaGetServerNames = function () { return Object.keys(serverConfigs); }; // Export server names for Umbrella
